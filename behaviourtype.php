...
/**
 * Question behaviour type for deferred feedback with CBM behaviour for MooPT questions.
 *
...
/**
 * Question behaviour type information for deferred feedback with CBM behaviour for MooPT questions.
 *
 * @copyright  2012 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbehaviour_deferredmooptcbm_type extends qbehaviour_deferredmoopt_type {
    ...
    public function summarise_usage(question_usage_by_activity $quba, question_display_options $options) {
        ...
        $summarydata['qbehaviour_cbm_grade_explanation'] = array(
            'title' => '',
            'content' => html_writer::tag('i', get_string('cbmgradeexplanation', 'qbehaviour_deferredmooptcbm')) .
                    $OUTPUT->help_icon('cbmgrades', 'qbehaviour_deferredmooptcbm'),
        );
        ...
        if ($notansweredcount && array_sum($attemptcount) > 0) {
            ...
            $summarydata['qbehaviour_cbm_answered_quiz_heading'] = array(
                'title' => '',
                'content' => html_writer::tag('h3',
                        get_string('foransweredquestions', 'qbehaviour_deferredmooptcbm', $totalquestions),
                        array('class' => 'qbehaviour_deferredmooptcbm_summary_heading')),
            );
        ...
        $summarydata['qbehaviour_cbm_judgement_heading'] = array(
            'title' => '',
            'content' => html_writer::tag('h3', get_string('breakdownbycertainty', 'qbehaviour_deferredmooptcbm'),
                    array('class' => 'qbehaviour_deferredmooptcbm_summary_heading')),
        );
        ...
        foreach ($attemptcount as $certainty => $count) {
            ...
            if ($count == 0) {
                $summarydata[$key] = array(
                    'title' => $title,
                    'content' => get_string('noquestions', 'qbehaviour_deferredmooptcbm'),
                );
                continue;
            }
            ...
            $a->fraction       = html_writer::tag('span', $this->format_probability($fraction),
                    array('class' => 'qbehaviour_deferredmooptcbm_actual_percentage'));
            ...
            $a->judgement = html_writer::tag('span', get_string($judgement, 'qbehaviour_deferredmooptcbm'),
                    array('class' => 'qbehaviour_deferredmooptcbm_' . $judgement));

            $summarydata[$key] = array(
                'title' => $title,
                'content' => get_string('judgementsummary', 'qbehaviour_deferredmooptcbm', $a),
            );
        }
